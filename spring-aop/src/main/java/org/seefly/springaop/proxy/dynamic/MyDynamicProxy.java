...
public class MyDynamicProxy implements InvocationHandler {
    ...
    /**
     * InvocationHandler是由代理实例的调用处理程序实现的接口。 每个代理实例都有一个关联的调用处理程序。在代理实例上调用方法时，方法调用将被编码并调度到其调用处理程序的invoke方法。
     * <p>
     * 控制对目标对象的访问. 上面根据接口动态生成的类长这个样子 其中的h 就是创建代理实例的时候传入的当前类
     * <p>
     * public final class $Proxy1 extends Proxy implements Call
     * <p>
     * //在内存中生成的class会有一个这样的构造函数，并且在Proxy.newProxyInstance(..)可以看到 //我们传进去的InvocationHandle会被当作构造参数来构建这个动态生成的类 public
     * $Proxy1(InvocationHandler var1) throws  { super(var1); }
     * <p>
     * // 然后我们在调用动态生成的类的方法时会执行InvocationHandle.invoke(..) public final void call(String var1) throws  { try {
     * super.h.invoke(this, m3, new Object[]{var1}); } catch (RuntimeException | Error var3) { throw var3; } catch
