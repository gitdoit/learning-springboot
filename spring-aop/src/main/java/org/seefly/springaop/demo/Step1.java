...
/**
 * 演示Spring 1.2中怎么使用AOP 整个流程很简单，需要AOP要有以下几个元素 1、通知 2、被代理对象 3、创建代理 这样我们创建代理，将被代理对象设置进去，再指定通知。 我们在使用的时候使用这个代理即可
 * <p>
 * 组件介绍 {@link LogAfterAdvice} 通知，我们需要织入的逻辑 {@link UserService} 需要被增强的对象 {@link ProxyFactoryBean}
 * 代理对象，给他设置通知，被代理对象，接口。他会应用通知对被代理对象进行增强 形成一个代理对象放到容器中，使用的时候使用这个代理对象即可
 * <p>
 * <p>
 * <p>
 * 缺陷 1、可以看到，UserService需要代理，我们就要手动创建一个代理对象，放到容器里。 用的时候从容器中取出来。如果多个对象都需要代理，我们需要手动的一个一个的去指定。很麻烦
 * 2、通知的粒度只细化到类上，类中所有的方法均被拦截了 3、获取的时候需要获取代理对象，不能直接使用被代理对象
 * <p>
 * 为了解决这种问题，请往下看
 * <p>
 * <p>
